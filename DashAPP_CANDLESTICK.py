...
# live_data.index = pd.to_datetime(live_data.index, unit="ms")
...
def findOrderBlocks(df, maxDistanceToLastBar, swingLength, obEndMethod, maxOrderBlocks):
    print(" DF SIZE IN OB "+str(df.size))
...
app.layout = html.Div([
    html.H1("Live Candlestick Chart"),
    dcc.Graph(id='live-candlestick-chart'),
    dcc.Interval(id='interval-component', interval=1 * 200, n_intervals=0)
])
...
def update_graph(n):
    ...
    #liveDataSize = live_data["close"].size
    #print(live_data["close"][liveDataSize-1])
    swing_length = 10
    maxOrderBlocks = 30
