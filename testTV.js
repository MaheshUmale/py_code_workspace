(() => {
    ...
    function i(r) {
        ...
        class g {
            ...
            _replaceScript(i) {
                ...
                const g = s.nonce || s.getAttribute("nonce"),
                    ...
                    m = c.querySelector(".tradingview-widget-copyright");
                this._copyrightContainer = p || m;
                const f = c.classList.contains("tradingview-widget-container");
                this.iframeContainer = c && f ? c : document.createElement("div"), o && (this.settings = this.filterRawSettings(o)), o && this._validateSettings() || (console.error("Invalid settings provided, fall back to defaults"), this.settings = this.filterRawSettings(this.defaultSettings));
                const w = "32px",
                    {
                        width: y,
                        height: b
                ...
                g && _.setAttribute("nonce", g), this.iframeContainer.appendChild(_);
                const C = this.hasCopyright() ? `calc(100% - ${w})` : "100%",
                    T = location.hostname,
                    k = m ? "widget_new" : "widget",
                    A = this.widgetUtmName();
                this.settings.utm_source = T, this.settings.utm_medium = k, this.settings.utm_campaign = A, this._updateCopyrightHrefParams(T, k, A);
                const I = this.settings.iframeTitle || `${this.widgetId.replace("-", " ")} TradingView widget`,
                    E = this.settings.iframeLang || "en";
                this.iframe = this._createIframe(C, h, a, I, E), this._addCSPErrorListener(h), g && this.iframe.setAttribute("nonce", g);
                const P = this.iframeContainer.querySelector(".tradingview-widget-container__widget");
                if (P ? ((0, t.ensureNotNull)(P.parentElement).replaceChild(this.iframe, P), s?.remove()) : f ? (this.iframeContainer.appendChild(this.iframe), s?.remove()) : (this.iframeContainer.appendChild(this.iframe), c.replaceChild(this.iframeContainer, (0, t.ensureNotNull)(s))), this.shouldListenToIframeResize && function(t, e, i) {
                    ...
                    t.style.height = w, t.style.lineHeight = w, void 0 !== v && (t.style.width = v), t.style.textAlign = "center", t.style.verticalAlign = "middle", t.innerHTML = p.innerHTML, this.iframeContainer.appendChild(t)
                }
            }
            _iframeSrcBase(t) {
            ...
            _setSettingsQueryString(t) {
                const e = this.propertiesToAddToGetParams.filter((t => -1 !== c.indexOf(t))),
                    i = function(t, e) {
            ...
            _setHashString(t, e) {
                ...
                })), (this.useParamsForConnectSocket || this.useSnowplowPageView) && (i["page-uri"] = h());
        ...
        let f;
        try {
            f = new URL(m.iframe.src).origin
        } catch (t) {
            f = "tradingview-widget.com"
        }
        window.addEventListener("message", (t => {
            if (t.origin !== f) return;
            t.stopImmediatePropagation();
            let e = t.data;
            if ("string" == typeof t.data && t.data.length && (e = JSON.parse(t.data)), "openChartInPopup" !== e.name) return;
            const i = JSON.parse(decodeURIComponent(e.data.url).slice(e.data.url.indexOf("#") + 1)),
                r = i.popup_width,
                s = i.popup_height,
                n = "string" == typeof r ? parseInt(r) : r,
                o = "string" == typeof s ? parseInt(s) : s,
                a = (screen.width - n) / 2,
                l = (screen.height - o) / 2,
                h = window.open(e.data.url, "_blank", `resizable=yes, top=${l}, left=${a}, width=${n}, height=${o}`);
            h && (h.opener = null)
        }))
    })()
})();